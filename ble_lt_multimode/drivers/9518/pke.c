				...
				unsigned int bWordLen, unsigned int *c)
{
	...
	{
		aWordLen = valid_words_get(a, aWordLen);
		tmemcpy(c, a, aWordLen<<2);
		tmemset(c+aWordLen, 0, (bWordLen-aWordLen)<<2);

		return PKE_SUCCESS;
	}
	else if(0 == ret)
	{
		tmemset(c, 0, bWordLen<<2);
	...
	if(bitLen)
	{
		...
		tmemcpy(p, a+bWordLen-1, tmpLen<<2);
		div2n_u32(p, tmpLen, bitLen);
		if(tmpLen < bWordLen)
		{
			tmemset(p+tmpLen, 0, (bWordLen-tmpLen)<<2);
		}
	...
	else
	{
		...
		{
			tmemcpy(a_high, a+bWordLen, tmpLen<<2);
			tmemset(a_high+tmpLen, 0, (bWordLen-tmpLen)<<2);
		}
	}
