 ...
 * @return     none
 */
static inline void gpio_ds_en(gpio_pin_e pin)
{
	unsigned char	bit = pin & 0xff;
 ...
 * @return     none
 */
static inline void gpio_ds_dis(gpio_pin_e pin)
{
	unsigned char	bit = pin & 0xff;
	unsigned short group = pin & 0xf00;
	if(group == GPIO_GROUPC)
 ...
 * @return     none
 */
static inline void gpio_set_output_en(gpio_pin_e pin, unsigned int value)
{
	unsigned char	bit = pin & 0xff;
	if(!value){
		BM_SET(reg_gpio_oen(pin), bit);
