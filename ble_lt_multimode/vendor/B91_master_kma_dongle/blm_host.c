...
int blm_le_adv_report_event_handle(u8 *p)
{
	...
	#if 0   //@@@@@  just for lijing debug, do not connect other device
		u8 specific_mac[4] = {0x82, 0x55, 0x55, 0x55};
		if( memcmp(pa->mac + 2, specific_mac, 4) ){  //not equal
			return 0;  //no connect
		}

//		user_manual_paring = 1;

//		if(pa->event_type == ADV_REPORT_EVENT_TYPE_SCAN_RSP){
//			my_dump_str_data (UDB_STR_DATA_ENABLE, "adv_report_SCAN_RSP", pa, pa->len + 11);// LLJ
//		}
//		if(pa->event_type == ADV_REPORT_EVENT_TYPE_ADV_IND){
//			my_dump_str_data (UDB_STR_DATA_ENABLE, "adv_report_ADV_IND", pa, pa->len + 11);// LLJ
//		}
	#endif
