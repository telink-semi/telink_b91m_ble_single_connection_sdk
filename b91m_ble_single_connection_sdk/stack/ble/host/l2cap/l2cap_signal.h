...
#define L2CAP_COC_STAT_NONE        0x00
#define L2CAP_COC_STAT_USED        0x01
#define L2CAP_COC_STAT_CONNECT     0x02
#define L2CAP_COC_STAT_PENDING     0x03
#define L2CAP_COC_STAT_DISCONN     0x04

#define L2CAP_COC_OPTS_START_SERVICE    0x01
#define L2CAP_COC_OPTS_RECONFIG         0x02

#define L2CAP_COC_REASON_COMMAND_NOT_UNDERSTOOD   0x0000
#define L2CAP_COC_REASON_SIGNALING_MTU_EXCEEDED   0x0001
#define L2CAP_COC_REASON_INVALID_CID_IN_REQUEST   0x0002
#define L2CAP_COC_REASON_UNKNOWN                  0x0003
 ...
 */
typedef int(*blc_l2cap_coc_datacb)(u16 connHandle, u16 scid, u16 dcid, u16 dataLen, u8 *pData);
...
typedef struct{
	...
	u8 nowChs;
	u8 *pRxBuffer; //Sdu RX Buffer
	u8 *pTxBuffer; //Sdu TX Buffer

	u16 newMtu;
	u16 newMps;

	u16 spsm;
	u16 rsdv;//u16 connHandle;
 ...
 */
ble_sts_t  blc_l2cap_cocInit(u8 connCount, u8 serviceCount, u8 channelCount, u8 *pBuffer, u16 buffLen);
 ...
 * @param[in]	pRxBuffer - A buffer to store data. You can use BLC_L2CAP_COC_MTU_NEEDLEN to calculate the required size.
 * @param[in]	pTxBuffer - A buffer to store data. You can use BLC_L2CAP_COC_MTU_NEEDLEN to calculate the required size.
...
int blc_l2cap_registerService(u16 spsm, u8 srvCount, u8 chCount, u16 mtu, u16 mps, 
	u16 credits, u8 *pRxBuffer, u8 *pTxBuffer, blc_l2cap_coc_evtcb evtCb, blc_l2cap_coc_datacb dataCb);
/**
 ...
 * @return		0: success;
 *             -1: The connHandle is not in a connected state;
 ...
 */
int blc_l2cap_startService(u16 connHandle, u16 spsm, u8 method);
/**
 ...
 */
int blc_l2cap_stopService(u16 connHandle, u16 spsm);
/**
 ...
 */
int blc_l2cap_disconnScid(u16 connHandle, u16 scid);
 ...
 */
int blc_l2cap_reconfigService(u16 connHandle, u8 srvNum, u16 mtu, u16 mps);
 ...
 */
u8  blc_l2cap_getSrvnumBySpsm(u16 connHandle, u16 spsm);
 ...
 *             >0: Failed to push data to Controller. Refer to ble_sts_t.
 */
int blc_l2cap_cocSendData(u16 connHandle, u16 scid, u8 *pHead, u8 headLen, u8 *pData, u16 dataLen);
 ...
 * @return
 */
int blc_l2cap_sendCommandRejectRsp(u16 connHandle, u8 identifier, u16 reason, u8 dataLen, u8 *pData);

int blc_l2cap_sendDisconnectReq(u16 connHandle, u8 identifier, u16 dcid, u16 scid);

int blc_l2cap_sendDisconnectRsp(u16 connHandle, u8 identifier, u16 dcid, u16 scid);

int blc_l2cap_sendLECreditBasedConnectReq(u16 connHandle, u8 identifier, u16 spsm,
                                          u16 scid, u16 mtu, u16 mps, u16 credits);

int blc_l2cap_sendLECreditBasedConnectRsp(u16 connHandle, u8 identifier, u16 dcid,
                                          u16 mtu, u16 mps, u16 credits, u16 result);

int blc_l2cap_sendFlowControlCreditInd(u16 connHandle, u8 identifier, u8 cid, u8 credits);

int blc_l2cap_sendCreditBasedConnectReq(u16 connHandle, u8 identifier, u16 spsm, u16 mtu,
                                        u16 mps, u16 credits, u8 numCids, u16 *pSCidList);

int blc_l2cap_sendCreditBasedConnectRsp(u16 connHandle, u8 identifier, u16 mtu, u16 mps,
                                        u16 credits, u16 result, u8 numCids, u16 *pDCidList);

int blc_l2cap_sendCreditBasedReconfigReq(u16 connHandle, u8 identifier, u16 mtu, u16 mps,
                                         u8 numCids, u16 *pDCidList);

int blc_l2cap_sendCreditBasedReconfigRsp(u16 connHandle, u8 identifier, u16 result);
